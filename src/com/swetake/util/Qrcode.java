...
//import com.sun.cldc.io.BufferedInputStream;
...
8264,8920,9368,9848,10288,10832,11408,12016,12656,13328}
    ...
    if (qrcodeVersion==0){
        /* auto version select */
    	System.out.println("calQrcode: qrCodeVer is autoselected");
        qrcodeVersion=1;
        for (int i=1;i<=40;i++){
        ...
        System.out.println("calQrcode: qrCodeVer is selected to "+qrcodeVersion);
    } else {
    	System.out.println("calQrcode: qrCodeVer is specified to "+qrcodeVersion);
        maxDataBits=maxDataBitsArray[ec][qrcodeVersion];
    }
    System.out.println("calQrcode: maxDataBits is specified to "+maxDataBits);
    totalDataBits+=codewordNumPlus[qrcodeVersion];
    dataBits[codewordNumCounterValue]+=codewordNumPlus[qrcodeVersion];
    System.out.println("calQrcode: totalDataBits is specified to "+totalDataBits);
...
/* ---- read version ECC data file */
	...
	//BufferedInputStream bis = new BufferedInputStream(fis);
	    ...
	    fis.read(matrixY);
	    fis.read(maskArray);
	    fis.read(formatInformationX2);
	    fis.read(formatInformationY2);
	    fis.read(rsEccCodewords);
	    fis.read(rsBlockOrderTemp);
	    fis.close();
	fis.close();
    } catch(Exception e) {
        e.printStackTrace();
...
/*  --- set terminator */
    ...
    if (totalDataBits<=maxDataBits-4){
	    ...
	    dataValue[dataCounter]=0;
            ...
            if (totalDataBits>maxDataBits){
                System.out.println("overflow: totalDataBits="+totalDataBits+","+
                					"maxDataBits="+maxDataBits+","+
                					"qrcodeVersion="+qrcodeVersion+","+
                					"qrcodeErrorCorrect="+ec);
            }
        }
    }
...
private static byte[] calculateRSECC(byte[] codewords, byte rsEccCodewords,byte[] rsBlockOrder,int maxDataCodewords,int maxCodewords){
    ...
    try {
        String filename = QRCODE_DATA_PATH+"/rsc"+String.valueOf(rsEccCodewords)+".dat";
		System.out.println("filename is " + filename);
		
		Qrcode qr = new Qrcode();
        //InputStream fis =Qrcode.class.getResourceAsStream(filename);
	InputStream fis =qr.getClass().getResourceAsStream(filename);
	//BufferedInputStream bis = new BufferedInputStream(fis);
            for (int i=0;i<256;i++){
...
//        byte[] rsTempData=(byte[])rsTemp[rsBlockNumber].clone();
        rsTempData=(byte[])doClone(rsTemp[rsBlockNumber]);
	...
	while(j>0){
		...
		byte[] cal=rsCalTableArray[(first & 0xFF)];
	            ...
	            System.arraycopy(rsTempData,1,rsTempNew,0,rsTempData.length-1);
                    rsTempData =(byte[])doClone(rsTempNew);
                } else {
...
private static byte[] doClone(byte[] src) {
	if (src != null) {
